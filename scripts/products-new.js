// Remove Elements that are conditionally hidden by webflow
...
// initiate states
// Add active state to first lifestyle and pre-set radio button selection
// $("#Configurator-LeadGen").submit(function () {
//   configValueBtn.eq(0).click();
// });

$(".config_tab-link:visible").last().addClass("is-end");

// Make user make a selction from each option set

// Make all option sets except for the first, inactive
$(".config_item:not(:first)").addClass("inactive");

// when selecting an option , remove inactive state from next option set
$(".config-value").click(function () {
  let parent = $(this).parents(".config_item");
  console.log(parent);
  let parentIndex = $(".config_item").index(parent);
  $(".config_item")
    .eq(parentIndex + 1)
  ...
  if (
    $(".config_item")
      .last()
      .find(".config-value.single-choice.foundations.active").length !== 0
  ) {
    $("#adu-config-btn-1").removeClass("disabled");
  }
});

$("#adu-config-btn-1").on("mouseenter", function () {
  if ($(this).hasClass("disabled")) {
    $(".instr-popup").addClass("active");
    setTimeout(() => {
      $(".instr-popup").removeClass("active");
    }, 5000);
  }
});
...
// add minimum of $2,500 per truckload
...
function driveDistanceCallback(status, milesDistance) {
  if (status === "OK") {
...
// disable form submit until all fields validated
$(document).ready(function () {
  $("#contact-info-submit").prop("disabled", true);
});
...
$("#LeadGen-Form")
  ...
  .change(function () {
    if (aduForm.valid() === true) {
      $("#contact-info-submit").prop("disabled", false);
    }
  });
...
function calcDelivery(status, miles) {
  let mileCost;
  if (productHeaderNoSpace === "shift+" || productHeaderNoSpace === "model6") {
    mileCost = 10;
  } else {
    mileCost = 7;
  }
  deliveryPrice = miles * mileCost * truckloads;
  let baseDeliveryPrice = 5000 * truckloads;
  // console.log("delivery price= ", deliveryPrice);
  // console.log("miles= ", miles);
  // console.log("mile cost= ", mileCost);
  // console.log("truckloads= ", truckloads);
  if (deliveryPrice < baseDeliveryPrice) {
    deliveryPrice = baseDeliveryPrice;
    $("#summ-freight-price")
      .text(
        baseDeliveryPrice.toLocaleString("en-US", {
          style: "currency",
          currency: "USD"
        })
      )
      .slice(0, -3);
    $("#estimated-delivery").val(baseDeliveryPrice);
  } else {
    // console.log("Delivery price= ", deliveryPrice);
    $("#estimated-delivery").val(deliveryPrice);
    $("#summ-freight-price")
      .text(
...
$("#Configurator-LeadGen").submit(function () {
  ...
  if ($("#Project-State").val() === "CA") {
    ...
    // $("#adu-config-btn-1").prop("disabled", true);
  }
...
function updateButtonText() {
  ...
  if (btnText === "Summary") {
    $("#adu-config-btn-1").addClass("disabled");
  }
}
...
function getImages() {
  Object.keys(imageUrl).forEach((image, index) => {
    ...
    // format url
    if (/webp/i.test(modString) || /jpeg/i.test(modString)) {
      imageUrl[`${image}`] = modString.slice(5, modString.length - 2);
    } else if (
      /jpg/i.test(modString) ||
      /png/i.test(modString) ||
      /gif/i.test(modString)
    ) {
      imageUrl[`${image}`] = modString.slice(5, modString.length - 2);
    }
  });
